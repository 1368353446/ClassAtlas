...
- **可观测 + 可缓存**：每个流水线阶段都会写日志（含 LLM 请求/响应摘要），同时自动检测产物以决定是否跳过某个阶段，复用已有字幕、梳理或幻灯片。
...
- 导入 `app` 时会立即配置日志格式（`时间 [级别] logger: 内容`）。设置 `APP_LOG_LEVEL=DEBUG` 可查看更详细的流水线信息，设置 `APP_LOG_DIR=/path/to/logs` 可将日志持久化到文件。
- 每次调用大模型都会记录模板、主要参数（字符长度）以及部分响应内容；流水线阶段、幻灯片对齐、翻译等动作也会写入 INFO 日志，便于审计。

## 产物感知的重复运行

- `run_pipeline` 会在执行每个阶段前检查目标文件：
  - 已有 `transcript_segments.json` 时跳过 Whisper。
  - 已有 `transcript_outline.txt` + outline JSON 时跳过梳理阶段。
  - 幻灯片 manifest 复用，除非 PDF 发生变化；知识点与幻灯片的关联只在必要时重新计算。
- 可运行 `scripts/test_pipeline_cache.py` 来快速验证缓存逻辑，而不用动真实讲座数据。
